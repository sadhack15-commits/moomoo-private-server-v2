...
app.get("/", (req, res) => {
    res.send("Moomoo private server is running.");
});
            ...
            try {
                ...
                if (type == Packets.CLIENT_TO_SERVER.JOIN_GAME) {
                    ...
                    if (type == Packets.CLIENT_TO_SERVER.PING_SOCKET) {
                        ...
                        } else if (data[0].startsWith("!ime ")) {
                            const botPlayer = new Player();
                            players.push(botPlayer);

                            botPlayer.setUserData({
                                name: `instakillerman`,
                                skin: 0
                            });

                            botPlayer.spawn();
                            botPlayer.resetResources();

                            botPlayer.mode = "instakiller";
                            botPlayer.state = -3;

                            botPlayer.x = player.x + 80;
                            botPlayer.y = player.y;
                            botPlayer.dir = Math.PI;

                            botPlayer.weaponIndex = 5;
                            botPlayer.skinIndex = 6;

                            const i = data[0].split(" ")[1];
